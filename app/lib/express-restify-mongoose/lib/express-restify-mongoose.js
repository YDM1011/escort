...
var restify = function restify(app, model) {
  ...
  var processFields = require('./middleware/processFields');
  ...
  if (!_.isArray(options.preCustomLink)) {
    options.preCustomLink = options.preCustomLink ? [options.preCustomLink] : [];
  }
  ...
  var dropDownList = uriItems + '/dropDownList';
  var uriShallow = uriItem + '/shallow';
  var validateUrl = uriItems + '/validate';
  var customLinkItemsUrl = uriItems + '/customLinkList';

  console.log(uriItem, uriItems, uriCount)
  ...
  var deletedMiddleware = [];

  if (typeof model.schema.paths.deleted !== "undefined") {
    deletedMiddleware = [function (req, res, next) {
      ...
      if (req.query.query) {
        ...
        req.query.query = JSON.stringify({deleted: false});
      }
      next();
    }];
  }

  if (backendApp.customEndPoints.indexOf(options.name.toLowerCase() + '.list') === -1) {
    app.get(uriItems, deletedMiddleware, prepareQuery, options.preMiddleware, options.preRead, accessMiddleware, ops.getItems, processFields, prepareOutput);
  }
  app.get(dropDownList, deletedMiddleware, prepareQuery, options.preMiddleware, accessMiddleware, ops.getItems, processFields, prepareOutput);
  app.get(customLinkItemsUrl, deletedMiddleware, prepareQuery, options.preMiddleware, options.preCustomLink, accessMiddleware, ops.getCustomLinkItems, prepareOutput);
  app.get(uriCount, deletedMiddleware, prepareQuery, options.preMiddleware, options.preRead, accessMiddleware, ops.getCount, prepareOutput);
  app.get(uriItem, deletedMiddleware, prepareQuery, options.preMiddleware, options.preRead, accessMiddleware, ops.getItem, processFields, prepareOutput);
  app.get(uriShallow, deletedMiddleware, prepareQuery, options.preMiddleware, options.preRead, accessMiddleware, ops.getShallow, prepareOutput);
  ...
  app.post(validateUrl, prepareQuery, ensureContentType, options.preMiddleware, accessMiddleware, ops.validateObject, prepareOutput);
  ...
  if (backendApp.customEndPoints.indexOf(options.name.toLowerCase() + '.delete') === -1) {
